...
type StaffFulfillment = {
  id: string;
  staffRequirementId?: string;
  staffId: string;
  startAt?: string | null;
  endAt?: string | null;
  createdAt?: string;
...
export function StaffFulfillments({ requirementId }: { requirementId: string }) {
  const queryClient = useQueryClient();
  const [page, setPage] = useState(1);
  const pageSize = 10;

  const q = useQuery({
    queryKey: ["staff", requirementId, "fulfillments"],
    queryFn: async (): Promise<StaffFulfillment[]> => {
      const res = await api.get(`/requirements/staff/${requirementId}/fulfillments`);
      return res.data;
    },
  ...
  const createMut = useMutation({
    mutationFn: (body: { staffId: string; startAt?: string; endAt?: string }) =>
      api.post(`/requirements/staff/${requirementId}/fulfillments`, body),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["staff", requirementId, "fulfillments"] }),
  });

  const updateMut = useMutation({
    mutationFn: (payload: { fid: string; body: { startAt?: string | null; endAt?: string | null } }) =>
      api.patch(`/requirements/staff/fulfillments/${payload.fid}`, payload.body),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["staff", requirementId, "fulfillments"] }),
  });
...
function CreateStaffFulfillmentForm({ onSubmit }: { onSubmit: (v: { staffId: string; startAt?: string; endAt?: string }) => void }) {
  ...
  return (
    <div className="flex flex-col gap-3 py-2">
      ...
      <div className="flex justify-end">
        <Button onClick={() => onSubmit({ staffId, startAt, endAt })}>Create</Button>
      </div>
    </div>
  );
}

function EditStaffFulfillmentForm({ initial, onSubmit }: { initial: StaffFulfillment; onSubmit: (v: { startAt?: string | null; endAt?: string | null }) => void; }) {
