...
export default function Robots() {
  ...
  const { data: firebaseRobots, isLoading, error } = useQuery({
    ...
    // If Firebase fetch fails, use our sample data
    onError: (err) => {
  ...
  // Use either the fetched data or our sample data
  const robots = firebaseRobots || robotsData;

  useEffect(() => {
    // Calculate robot status counts from real or sample data
    const robotsToUse = robots || robotsData;
    const active = robotsToUse.filter((robot) => robot.status === "Active").length;
    const charging = robotsToUse.filter((robot) => robot.status === "Charging").length;
    const maintenance = robotsToUse.filter((robot) => robot.status === "Maintenance").length;
